 ...
 */
class puzzle_message_Request extends puzzle_message_AbstractMessage implements puzzle_message_RequestInterface
{
    ...
    /** @var puzzle_Url HTTP Url */
    private $url;

    /** @var string HTTP method */
    private $method;

    /** @var puzzle_Collection Transfer options */
    private $transferOptions;

    /**
     * @param string                         $method  HTTP method
     * @param string|puzzle_Url       $url     HTTP URL to connect to. The URI scheme,
     *   host header, and URI are parsed from the full URL. If query string
     *   parameters are present they will be parsed as well.
     * @param array|puzzle_Collection $headers HTTP headers
    ...
    public function __construct(
        ...
        $this->transferOptions = new puzzle_Collection($options);
    ...
    private function addPrepareEvent()
    {
        static $subscriber;
        if (!$subscriber) {
            $subscriber = new puzzle_subscriber_Prepare();
        }
