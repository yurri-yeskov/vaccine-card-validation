 ...
 * Post file upload
 */
class puzzle_post_PostFile implements puzzle_post_PostFileInterface
{
    private $name;
    private $filename;
    private $content;
    ...
    private function prepareFilename($filename)
    {
        ...
        if (!$this->filename &&
            $this->content instanceof puzzle_stream_MetadataStreamInterface
        ) {
    ...
    private function prepareDefaultHeaders()
    {
        ...
        if (!$this->hasHeader('Content-Type')) {
            $this->headers['Content-Type'] = puzzle_Mimetypes::getInstance()
                ->fromFilename($this->filename) ? puzzle_Mimetypes::getInstance()
    ...
    {
        $a = array_change_key_case($this->headers);
        return isset($a[strtolower($name)]);
    }
}
