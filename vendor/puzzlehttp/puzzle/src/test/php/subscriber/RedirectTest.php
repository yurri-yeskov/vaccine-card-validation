...
/**
 * @covers puzzle_subscriber_Redirect
 */
class puzzle_test_subscriber_RedirectTest extends PHPUnit_Framework_TestCase
{
    public function testRedirectsRequests()
    {
        $mock = new puzzle_subscriber_Mock();
        $history = new puzzle_subscriber_History();
        $mock->addMultiple(array(
    ...
    /**
     * @expectedException puzzle_exception_TooManyRedirectsException
    ...
    public function testDefaultBehaviorIsToRedirectWithGetForEntityEnclosingRequests()
    {
        ...
        $client->post('http://test.com/foo', array(
    ...
    public function testRewindsStreamWhenRedirectingIfNeeded()
    {
        ...
        $body = $this->getMockBuilder('puzzle_stream_StreamInterface')
            ->setMethods(array('seek', 'read', 'eof', 'tell'))
        ...
        $client->post('/foo', array(
            'body' => $body,
            'allow_redirects' => array('max' => 5, 'strict' => true)
        ));
    }

    /**
     * @expectedException puzzle_exception_CouldNotRewindStreamException
    ...
    public function testThrowsExceptionWhenStreamCannotBeRewound()
    {
        ...
        $body->expects($this->any())->method('eof')->will($this->returnValue(true));
        $body->expects($this->any())->method('read')->will($this->returnValue('foo'));
        $client->post('http://example.com/foo', array(
            'body' => $body,
            'allow_redirects' => array('max' => 10, 'strict' => true)
        ));
    }
    ...
    public function testAddsRefererWhenPossible()
    {
        ...
        $h = new puzzle_subscriber_History();
        $client->getEmitter()->attach($h);
        $client->get('/foo', array('allow_redirects' => array('max' => 5, 'referer' => true)));
        $reqs = $h->getRequests();
    ...
    {
        $client = new puzzle_Client(array('base_url' => 'https://www.foo.com'));
        $client->getEmitter()->attach(new puzzle_subscriber_Mock(array(
            "HTTP/1.1 301 Moved Permanently\r\n"
