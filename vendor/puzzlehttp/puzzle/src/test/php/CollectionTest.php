...
class puzzle_test_CollectionTest extends PHPUnit_Framework_TestCase
{
    /** @var puzzle_Collection */
    protected $coll;

    protected function setUp()
    {
        $this->coll = new puzzle_Collection();
    }
    ...
    public function testConstructorCanBeCalledWithParams()
    {
        ...
        $this->coll = new puzzle_Collection($testData);
    ...
    public function testAllowsFunctionalFilter()
    {
        ...
        $filtered = $this->coll->filter(array($this, '__callback_testAllowsFunctionalFilter'));
    ...
    public function testAllowsFunctionalMapping()
    {
        ...
        $mapped = $this->coll->map(array($this, '__callback_testAllowsFunctionalMapping'));
    ...
    {
        $c = puzzle_Collection::fromConfig(array(
            'a' => '123',
        ...
        $this->assertInstanceOf('puzzle_Collection', $c);
        $this->assertEquals(array(
            'a' => '123',
            'b' => 'lol',
            'base_url' => 'http://www.test.com/'
        ...
        try {
            $c = puzzle_Collection::fromConfig(array(), array(), array('a'));
    ...
    {
        $c = new puzzle_Collection(array('value' => $a));
    ...
    public function testRetrievesNestedKeysUsingPath()
    {
        ...
        $collection = new puzzle_Collection($data);
    ...
    {
        $collection = new puzzle_Collection(array(
            '0' => array(
                'a' => 'jar'
            ),
            1 => 'other'
     ...
     */
    public function testGetPath(puzzle_Collection $c, $path, $expected, $separator = '/')
    {
    ...
    {
        $c = new puzzle_Collection(array('foo' => 1, 'baz' => 2, 'bar' => 3));
    ...
    {
        $c = new puzzle_Collection(array('foo' => 1, 'baz' => 2, 'bar' => 3));
        $b = new puzzle_Collection(array('foo' => 10, 'bar' => 300));
        $c->overwriteWith($b);
        $this->assertEquals(array('foo' => 10, 'baz' => 2, 'bar' => 300), $c->toArray());
    }
    ...
    {
        $c = new puzzle_Collection(array('foo' => array('bar' => 'test')));
    ...
    {
        $c = new puzzle_Collection(array('foo' => 'bar'));
        $c->setPath('foo/bar', 'hi');
        $this->assertEquals('hi', $c['foo']['bar']);
    }
    ...
    {
        $c = new puzzle_Collection();
        $c->setPath('foo/bar/[]', 'a');
        $c->setPath('foo/bar/[]', 'b');
        $this->assertEquals(array('a', 'b'), $c['foo']['bar']);
    }
}
