...
start-server:
	@ps aux | grep 'node src/test/js/server.js' | grep -v grep > /dev/null \
	|| node src/test/js/server.js &> /dev/null &
...
test: start-server
	vendor/bin/phpunit -c src/test/resources/phpunit.xml.dist
	$(MAKE) stop-server
